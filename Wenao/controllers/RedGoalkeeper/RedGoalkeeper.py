...
class SoccerRobot(Robot):
    ...
    def __init__(self, config):
        ...
        self.PlayerMode = config.get("RedGoalkeeper", "PlayerMode")
        self.Strategy = config.get("RedGoalkeeper", "Strategy")
        TargetPosition = config.get("RedGoalkeeper", "TargetPos")
        self.TargetPosition = list(map(float, TargetPosition.split(",")))
        StartLocation = config.get("RedGoalkeeper", "StartPos")
            ...
            case RobotState.BE_A_GOALKEEPER:
                if self.Supervisor.data["GameStatus"]:
                    ...
                    if self.Supervisor.data["ballOwner"][0] != "R":
                        ...
                        # Zone Definitions
                        CLOSE_ZONE_RADIUS = 1.125
                        MID_ZONE_RADIUS = 2.25

                        isTheBallClose = Functions.calculateDistance(
                            currentBallPosition, [-4.5, 0.00]
                        )

                        if isTheBallClose <= CLOSE_ZONE_RADIUS:
                            targetAngle = np.degrees(
                                np.arctan2(
